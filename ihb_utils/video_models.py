...
class VideoMetrics:
    ...
    def from_ffprobe_data(cls, probe_data: Dict[str, Any], profile, is_set_bitrate: bool) -> VideoMetrics:
        from .video_utils import calc_bitrate, calc_bppf
        ...
        data["codec"] = f"{video_data["codec_name"]} x {audio_data.get('codec_name', 'NO AUDIO')} @ {int(audio_data.get('sample_rate', 0))//1000}k"
    ...
    def to_pretty_list(self) -> List[str]:
        from .video_utils import format_bitrate
    ...
    def compare_to(self, new_metrics: VideoMetrics) -> List[str]:
        ...
        comp_data[VideoMetrics.LABELS.index("Size")] = f"{format_size(size_delta)} : {size_reduction:.02f}%"
