...
import os
import re
from math import gcd, log10, modf
from typing import Any, Dict, List, Tuple

from humanfriendly import format_size

from ihb_encode.data.types import PROFILES, EncodingProfile
from ihb_ext.ffprobe import _get_stream_size

from .gen_utils import format_time
from .video_models import Resolution
...
def _build_chapter_file(file_path: str, probe_datas: List) -> str:
    ...
    with open(chapter_file_path, "w", encoding="utf-8") as f:
        for idx, probe_data in enumerate(probe_datas, 1):
            ...
            f.write(f"CHAPTER{idx:02d}NAME={os.path.splitext(os.path.basename(probe_data["format"]["filename"]))[0]}")
            cumulative_duration += probe_data["format"]["duration"]
...
def format_bitrate(bitrate: float) -> str:
    ...
    return f"{scaled:.2f}".rstrip("0").rstrip(".") + f" {prefixes[prefix_idx]}bps"
